...
class CableStraightArmPulldownExercise(Exercise):
    ...
    counter_rules: list[AngleCounterRule] = field(
        default_factory=lambda: [
            AngleCounterRule(
                ...
                rom_min_angle=20,              # must reach <= 60° for a GOOD rep (deep enough)
                rom_max_angle=100,             # must reach >= 160° for a GOOD rep (full extension)
    ...
    metadata: dict = field(
        default_factory=lambda: {
            ...
            "technique_notes": (
                "Keep arms completely straight throughout the movement (elbow angle >= 150°). "
                "Pull from overhead down to your sides using only shoulder movement. "
                "Avoid swinging or hip thrusting — keep torso stable. "
                "Full extension at the top (~160° - 180° overhead) and full pulldown at the bottom (60° - 70°). "
                "Controlled tempo — focus on lat contraction, not momentum."
            ),
        }
    )
